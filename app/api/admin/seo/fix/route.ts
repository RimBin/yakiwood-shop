...
import { suggestSeoFixes } from '@/lib/seo/runtime-scanner'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const fixes = await suggestSeoFixes({ origin })

    return NextResponse.json({ fixes, origin, generatedAt: new Date().toISOString() })
  } catch (e: any) {
