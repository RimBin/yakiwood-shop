...
function getStripeClient(): Stripe | null {
  ...
  return new StripeCtor(process.env.STRIPE_SECRET_KEY, { apiVersion: '2025-11-17.clover' }) as Stripe;
}
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      ...
      if (!emailResult.success) {
        ...
        console.log(
          `Order ${orderNumber || order.order_number} processed, invoice ${invoice.invoiceNumber} sent to ${customerEmail}`
        );
      }
    } catch (error) {
